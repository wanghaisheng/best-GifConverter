<template>
  <div class="base-table">
    <div class="base-table-operates">
      <Space>
        <Button size="small" v-for="item in operates" v-bind="{ ...item }">
          {{ item.text }}
        </Button>
      </Space>
      <Space style="color: rgb(var(--gray-10))">
        <span>共 </span>
        <span style="color: #2d5cf6">{{ data.length }} </span>
        <span>条数据</span>
        <span>/</span>
        <span>已选择 </span>
        <span style="color: #2d5cf6">{{ selectedKeys.length }} </span>
        <span>条</span>
      </Space>
    </div>
